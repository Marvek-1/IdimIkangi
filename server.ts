...
function calculateStats() {
  ...
  return {
    ...stats,
    unresolved: signals.filter((s: any) => !s.outcome).length,
...
app.get('/trade/exchanges', (req, res) => {
  res.json({
    active_exchanges: [
      { name: "binance", is_simulated: false },
      { name: "bybit", is_simulated: false },
      { name: "okx", is_simulated: false },
      { name: "bitget", is_simulated: false }
    ]
  });
});

app.get('/trade/positions', (req, res) => {
  res.json({ positions: [] });
});

app.get('/trade/balances', (req, res) => {
  res.json({
    balances: {
      binance: 12500.50,
      bybit: 8400.20,
...
app.get('/market/ticker/:exchange/:symbol', (req, res) => {
  const { symbol } = req.params;
  // Mock price data
  const basePrice = symbol.startsWith('BTC') ? 65000 : symbol.startsWith('ETH') ? 3500 : 150;
  const last = basePrice + (Math.random() * 10 - 5);
  res.json({
    symbol,
    last,
